...
var Chordd = function() {
	...
	//this.server = window.location.origin + '/Server/Chordd';
	this.server = 'https://serve-unrealk.herokuapp.com/Chordd';
	this.saves = [];
...
Chordd.prototype.refresh = function(sort) {
	...
	if(index.length < 1) {
		...
		//container.append('<div padded item fg-yellowA100>No saved data!</div>');
		...
		if(sort === "titles" || sort === "tabs" || sort === "chords") {
				...
				if(data.data) {
					...
					html += '<div>';
						...
						html += '<info>';
							...
							html += ' &mdash; <span fg-yellowA100>' + rating_count + ' votes</span>';
						html += '</info>';
			...
			for(var i=0; i<fullData.length; i++) {
				...
				if(artists.indexOf(artist)<0) {
					artists.push(artist);
					var elem = $('<a padded item fg-yellowA100 onclick="app.searchThis(this)">' + fullData[i].artist + '</a>');
					elem.append(html);
					frag.appendChild(elem[0]);
					count++;
				}
	...
	if(this.verify(json) === true) {
		...
		if(json.suggestions != null && json.suggestions.length > 0) {
			...
			for(var i=0; i<json.suggestions.length; i++) {
				var elem = $('<a padded item fg-yellowA100 onclick="app.searchThis(this)">' + json.suggestions[i] + '</a>');
				frag.appendChild(elem[0]);
			}
			container.append(frag);
		} else {
			// no suggestions
			//container.append('<div padded item fg-yellowA100>No suggestions</div>');
		}
	}
}
...
Chordd.prototype.parseResults = function(json) {
	if(this.verify(json) === true) {
		...
		if(json.results != null && typeof json.results === 'object') {
			frag.appendChild($('<div smaller bold padded fg-yellowA100 bg-teal800>&middot; CHORDS</div>')[0]);
			if(json.results.chords.length > 0) {
				for(var i=0, html, data; i<json.results.chords.length; i++) {
					data = json.results.chords[i];
					var title   = data.title || 'Unknown Title';
					var artist  = data.artist || 'Unknown Artist';
					var version = data.version || 1;
					var rating  = data.rating || 0;
			...
			frag.appendChild($('<div smaller bold padded fg-yellowA100 bg-teal800>&middot; TABS</div>')[0]);
			if(json.results.tabs.length > 0) {
				for(var i=0, html, data; i<json.results.tabs.length; i++) {
					data = json.results.tabs[i];
					var title   = data.title || 'Unknown Title';
					var artist  = data.artist || 'Unknown Artist';
					var version = data.version || 1;
					var rating  = data.rating || 0;
			...
			// no results
			frag.appendChild($('<a padded item fg-yellowA100>No results have been found.</a>')[0]);
		}
		container.append(frag);
	}
}
...
Chordd.prototype.request = function(type, data) {
	...
	if(type === 'suggestions' || type === 'results') {
		...
		$('[list-online]').append('<div padded fg-yellowA100>Loading...</div>');
		// check local items
		(function(data) {
			$.jStorage.reInit();
			var container  = $('[list-local]');
			...
			var key, data, html;
			if(index.length < 1) {
				//container.append('<div padded item fg-yellowA100>No saved data matched!</div>');
			} else {
				// document fragment for reflow optimizations
				// http://www.smashingmagazine.com/2012/11/05/writing-fast-memory-efficient-javascript/
				var frag = document.createDocumentFragment();
...
Chordd.prototype.display = function(data) {
	...
	var rating  = data.rating  || 0;
	var rating_count = data.rating_count || 0;
	var content = data.data || 'Unknown Content';
	$('[tab-title]').html(title);
	$('[tab-artist]').html(artist);
	var html = '<span inverted>';
	var star_full = '<span icon="star"></span>';
	var star_half = '<span icon="star-outline"></span>';
		switch(Math.floor(rating)) {
			case 1: html += star_full; html += star_half; html += star_half; html += star_half; html += star_half; break;
	...
	this.title = '<span fg-yellowA100>' + title + '</span>';
	// scroll to top
	$(window).scrollTop(0);
	// hide panel
