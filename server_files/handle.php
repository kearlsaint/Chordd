...
&& strtolower($_SERVER['HTTP_X_REQUESTED_FROM']) == 'application'
	...
	and isset($_POST['data_ratingcount'])){
		...
		$songid  = clean2($_POST['data_songid']);
		...
		$url = "https://tabs.ultimate-guitar.com/tab/".$artist."/".$title."-".$type."-".$songid;
		$data = file_get_contents($url);
		$data = between($data, '"js-store" data-content="', '"></div>');
		...
		$count_tab = between($page_tab, '"js-store" data-content="', '"></div>');
		$count_crd = between($page_crd, '"js-store" data-content="', '"></div>');
		...
		die(json_encode(array(
			"state" => "success",
			"results" => array(
				"chords" => html_entity_decode($count_crd),
				"tabs"   => html_entity_decode($count_tab),
			),
		)));
